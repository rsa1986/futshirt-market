...
export function CityStatePicker({ stateVal, cityVal, onStateChange, onCityChange, required }) {
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
      <div>
        <label style={{ fontSize:12,color:C.gray600,display:"block",marginBottom:4 }}>Estado{required&&<span> *</span>}</label>
      ...
      {stateVal && (
        <div>
          <label style={{ fontSize:12,color:C.gray600,display:"block",marginBottom:4 }}>
            Cidade{loadingCities ? " (carregando...)" : ""}{required&&<span> *</span>}
          </label>
          <input
