...
export default function App() {
  ...
  const emptyForm = { team:"",country:"",year:"",edition:"",condition:"Nova",price:"",price_old:"",size:"M",model:"",type:"times",region:"nacional",description:"",photos:[],
    status:"disponivel",acquisition_price:"",has_name:false,has_number:false,player_name:"",shirt_color:"",collection_note:"" };
  ...
  async function handleAddShirt() {
    ...
    const payload = {
      ...
      acquisition_price:parseFloat(form.acquisition_price)||null,
      has_name:form.has_name||false, has_number:form.has_number||false,
      player_name:form.player_name||null, shirt_color:form.shirt_color||null,
      collection_note:form.collection_note||null,
    };
  ...
  function startEditShirt(shirt) {
    setForm({
      ...
      status:shirt.status||"disponivel",
      acquisition_price:shirt.acquisition_price||"",
      has_name:shirt.has_name||false, has_number:shirt.has_number||false,
      player_name:shirt.player_name||"", shirt_color:shirt.shirt_color||"",
      collection_note:shirt.collection_note||"",
    });
              ...
              <div style={{ display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:10 }}>
                ...
                <div>
                  ...
                  <div style={{ display:"flex",gap:16 }}>
                    <label style={{ display:"flex",alignItems:"center",gap:6,cursor:"pointer",fontSize:14,color:C.gray700 }}><input type="checkbox" checked={form.has_name} onChange={e=>setForm(f=>({...f,has_name:e.target.checked}))} style={{ width:16,height:16 }} /> Nome</label>
                    <label style={{ display:"flex",alignItems:"center",gap:6,cursor:"pointer",fontSize:14,color:C.gray700 }}><input type="checkbox" checked={form.has_number} onChange={e=>setForm(f=>({...f,has_number:e.target.checked}))} style={{ width:16,height:16 }} /> Número</label>
                  </div>
                </div>
                <div style={{ gridColumn:"1/-1" }}>
  ...
  if(page==="portfolio") {
    ...
    const TABS = [["todos","📦 Todos",counts.todos],["colecao","📚 Coleção",counts.colecao],["venda","🏷️ À venda",counts.venda],["troca","🔄 Para troca",counts.troca],["historico","📜 Histórico",counts.historico]];
    ...
    return (
      <div style={{ fontFamily:"system-ui,sans-serif",maxWidth:1200,margin:"0 auto",padding:"0 0 4rem" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:isMobile?"repeat(3,1fr)":"repeat(5,1fr)",gap:10,marginBottom:16 }}>
          {[["📦",counts.todos,"Total","#374151","#f3f4f6"],["📚",counts.colecao,"Coleção","#1d4ed8","#eff6ff"],["🏷️",counts.venda,"À venda",C.greenDark,C.greenLight],["🔄",counts.troca,"Para troca","#7c3aed","#f5f3ff"],["📜",counts.historico,"Histórico",C.gray500,C.gray50]].map(([icon,n,label,color,bg])=>(
            <div key={label} style={{ background:bg,borderRadius:12,padding:"10px 8px",textAlign:"center",border:`1px solid ${color}22`,cursor:"pointer" }} onClick={()=>setPortfolioTab(label==="Total"?"todos":label==="Coleção"?"colecao":label==="À venda"?"venda":label==="Para troca"?"troca":"historico")}>
        ...
        {topClubs.length>0&&(
          <div style={{ background:C.white,border:`1px solid ${C.gray200}`,borderRadius:14,padding:"12px 16px",marginBottom:16 }}>
            <p style={{ margin:"0 0 10px",fontSize:12,fontWeight:700,color:C.gray700 }}>Por clube</p>
            <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
              {topClubs.map(([club,n])=>(
                <div key={club} onClick={()=>{ setPortfolioSearch(club); setPortfolioTab("todos"); }}
                  ...
                  <span style={{ fontSize:13,fontWeight:600,color:C.gray700 }}>{club}</span>
